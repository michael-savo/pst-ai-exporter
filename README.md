...
PST extraction always invokes ReadPST with `-j 0`. This is the only true-serial
mode in LibPST 0.6.76 and avoids the unstable message counts observed when its
internal worker processes are enabled. The manifest records the reader version,
arguments, and serial extraction mode used for the run.
...
`content_sha256` remains the hash of the unmodified raw EML bytes generated by
the reader. LibPST deliberately generates new MIME boundaries on each run, so
the exporter also records `semantic_sha256`, a deterministic identity over the
parsed evidence content. Stable IDs and duplicate detection use the semantic
identity while preserving every raw hash.

LibPST also generates paired calendar parts with the conversion time in
`DTSTAMP` and a random `i<number>.ics` filename. When a message contains a
matching inline/filename pair that identifies itself with `PRODID:LibPST`, the
exporter records both the raw attachment hash and a `semantic_sha256` that
ignores only those identified conversion artifacts. A standalone calendar
attachment is never normalized merely because it mentions LibPST. The raw file,
filename, size, and SHA-256 remain unchanged in the export, and every applied
normalization is disclosed in attachment metadata and warnings.
...
All selected archives feed one combined corpus. Evidence-content duplicates are
retained and marked with `duplicate_of`, allowing analysis tools to count or
omit them without losing evidence that the message appeared in more than one
source.
...
Use safe serial reading within each PST:
...
Do not increase `--jobs`. LibPST worker processes can change the number of
messages and attachments produced from the same PST. When higher throughput is
...
--jobs 0                     Required safe ReadPST mode; internal workers disabled
...
```json
{
  ...
  "content_sha256": "hash-of-reader-generated-raw-eml",
  "semantic_sha256": "stable-hash-of-parsed-evidence-content",
  "duplicate_of": null,
  "warnings": [],
  "source": {
    "id": "source-id",
  ...
  "attachments": [
    {
      "sha256": "raw-attachment-hash",
      "semantic_sha256": "stable-attachment-content-hash",
      "semantic_normalization": null
    }
  ],
